...
from app.services import RecipeServiceDep, NoFamilyException, NotFoundException

router = APIRouter(prefix="/recipes", dependencies=[Depends(auth.implicit_scheme)], tags=["recipes"])
...
async def suggest_tags(recipe_id: int, recipe_service: RecipeServiceDep):
    try:
        ...
        raise HTTPException(status_code=503, detail="Tag suggestion service is unavailable.")

@router.post("/{recipe_id}/suggested_substitutions", response_model=list[SuggestionDTO])
async def suggest_substitutions(recipe_id: int, ingredient_id: int, recipe_service: RecipeServiceDep):
    try:
        ...
        raise HTTPException(status_code=503, detail="Substitution suggestion service is temporarily unavailable.")